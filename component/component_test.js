steal("can", "can/map/define", "can/component", "can/view/stache" ,"can/route", "steal-qunit", function () {
	...
	test('something', function() {
		...
		var Component = can.Component.extend({
			tag: 'clicking-intensifies',
			template: can.stache((function(){/*!
			 <div>
			 <div can-click="{breakstuff}">
			 Click me to replace deepness.
			 </div>
			 {{#each deepness.rows}}
			 <div>{{.}}</div>
			 {{/each}}
			 </div>
			 */}).toString().replace(fcsRx,"")),
			scope: {
				define: {
					curId: { value: 1 },
					deepness: {
						get: function(cur) {
							var self = this;
							var getNewOne = !cur || cur.id !== this.attr( "curId" );

							console.log('Get', cur, getNewOne)
							if(getNewOne) {
								setTimeout( function(){
									self.attr( "deepness", {
										id: self.attr("curId"),
				...
				breakstuff: function() {
					this.attr( "curId", ~~( Math.random() * 1000 ) );
				}
			}
		});

		$('body').append(can.view.stache('<clicking-intensifies></clicking-intensifies>')({}));
	});
});
